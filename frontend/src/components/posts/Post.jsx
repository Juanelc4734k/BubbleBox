...
const Post = forwardRef((props, ref) => {
  ...
  const handleReaction = async (reactionType = "like") => {
    try {
      ...
      if (reactionType === "like") {
        const hasLike = updatedReactions.some(
          (r) => r.id_usuario === Number(userId) && r.tipo === "like"
        );
        setIsLiked(hasLike);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (reactions.length > 0) {
      ...
      const userReaction = reactions.find(
        (reaction) => reaction.id_usuario === Number(userId)
      );
      setIsLiked(!!userReaction);
      if (
        userReaction &&
        reactionTypes.some((r) => r.type === userReaction.tipo)
      ) {
  ...
  const shareUrl = `http://localhost:5173/posts/obtener/${
    post.id
  }?title=${encodeURIComponent(post.titulo || "")}&author=${encodeURIComponent(
    post.nombre_usuario || ""
  )}&type=bubblebox_post`;
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          ...
          text: `¡Descubre este post en BubbleBox! ${
            post.titulo || "Mira esta publicación"
          }: ${post.contenido || "Revisa contenido interesante en BubbleBox"}`,
          url: shareUrl,
        });
  ...
  const shareToFacebook = () => {
    const text = `¡No te pierdas esta publicación en BubbleBox! ${
      post.titulo || "Descubre esta increíble publicación"
    }: ${
      post.contenido || "Haz clic y conoce más sobre este tema interesante."
    }`;
    window.open(
  ...
  const shareToTwitter = () => {
    const text = `¡Descubre este post en BubbleBox! ${
      post.titulo || "Mira esta publicación"
    }: ${shareUrl}`;
    window.open(
  ...
  const shareToWhatsApp = () => {
    const text = `${
      post.titulo || "Mira esta publicación en BubbleBox"
  ...
  return (
    <>
        ...
        <div className="post">
            ...
            <div className="post-conten-img w-[90%] sm:w-[95%] xl:w-[95%] overflow-hidden rounded-md shadow-lg">
              {post.imagen && (
                <img
                  ...
                  onClick={() =>
                    imgAbrir(`http://localhost:3008/uploads/${post.imagen}`)
                  }
                />
              )}
            </div>
          ...
          <div className="post-aption flex justify-between items-center px-4 py-2">
            <div className="post-grup1 flex gap-6 relative">
              <div
                className={`post-reacciones flex items-center gap-2 cursor-pointer hover:text-blue-500 transition-colors ${
                  isLiked ? "text-blue-500" : ""
                }`}
              >
                <div
                  className="post-conten-reaction relative"
                ...
                >
                  {currentReaction && currentReaction !== "like" ? (
                    ...
                    <i
                      className={`fa-${
                        likedd ? "solid" : "regular"
                      } fa-thumbs-up text-xl cursor-pointer`}
                      onClick={(e) => {
                        e.stopPropagation();
                        setLikedd(!likedd);
                        handleReaction("like");
                ...
                {reactions &&
                  reactions.filter((reaction) => reaction.tipo === "like")
                    .length > 0 && (
              ...
              <div
                className="post-comentarios flex items-center gap-2 cursor-pointer"
                onClick={(e) => {
                  e.stopPropagation();
              ...
              >
                <i
                  className={`  fa-${
                    linkedComments ? "solid" : "regular"
                  } fa-regular fa-comment text-xl cursor-pointer`}
                ></i>
            ...
            <div className="grup2">
              <div
                className="post-reenviar relative cursor-pointer"
                onClick={handleShare}
      ...
      {imgaiAmplia && (
        <div
          className={`contImagenPost ${imgaiAmplia ? "imagen" : ""}`}
          onClick={imgCerrar}
          ...
          <div ref={editModalRef} className="edit-form-post">
            <div className="form-header-modal-post">
              <div className="edit-form-header">
                <h2 className="edit-text-publi">
                  <FiEdit className="edit-icono-publicacion" /> Editar
                  Publicación
                </h2>
                <button
              ...
              <div className="edit-time">
                {post.fecha_creacion && (
                  <div className="time-edit-2">
                    ...
                    <p>
                      {" "}
                      Las publicaciones solo pueden editarse dentro de las
                      primeras 24 horas
                    </p>
                  </div>
                )}
            ...
            <div className="modal-body-form-post">
              <div className="conten-edit-mensaje">
                {editMessage && (
                  <div
                    className={`edit-mensaje ${
                      editMessage.includes("Error") ||
                      editMessage.includes("No se puede")
                        ? "error"
                        : "success"
                    }`}
                  >
                ...
                {post.imagen && (
                  <div className="edit-image-preview">
                    <p className="text-sm text-gray-500 mb-2">
                      Imagen actual (<span>no se puede cambiar</span>):
                    </p>
                    <img
                      src={`http://localhost:3008/uploads/${post.imagen}`}
                      alt={post.titulo}
