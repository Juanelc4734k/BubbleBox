...
import { updatePrivacySettings, getUserSettings, updateUserSettings } from '../../services/users';
...
const Settings = () => {
        ...
        const fetchUserSettings = async () => {
            try {
                ...
                if (token) {
                    ...
                    console.log("Settings from backend:", userSettings);
                    ...
                    localStorage.setItem('userPrivacy', userSettings.privacidad || 'publico');
                    localStorage.setItem('notificationsEnabled', !!userSettings.notificaciones);
                    localStorage.setItem('audioEnabled', !!userSettings.audio_enabled);
                    localStorage.setItem('filesAccessEnabled', !!userSettings.files_access_enabled);
                    localStorage.setItem('onlineVisibilityEnabled', !!userSettings.online_visibility);
                    localStorage.setItem('language', userSettings.idioma || 'es');
                    localStorage.setItem('autoplayVideos', !!userSettings.autoplay_videos);
                } else {
    ...
    const handleToggleNotifications = async () => {
        try {
            ...
            toast.success(`Notificaciones ${newValue ? 'habilitadas' : 'deshabilitadas'}`);
        } catch (error) {
        ...
        try {
            ...
            if (newValue) {
                try {
                    ...
                    toast.warning('No se pudo acceder al micrófono. Verifica los permisos del navegador.');
                }
            }
            
            toast.success(`Grabación de audio ${newValue ? 'habilitada' : 'deshabilitada'}`);
        } catch (error) {
        ...
        try {
            ...
            if (newValue && window.showDirectoryPicker) {
                try {
                    ...
                    await window.showDirectoryPicker().then(() => {
                    ...
                    console.warn('File system access API not fully supported:', permissionError);
                }
            }
            
            toast.success(`Acceso a archivos ${newValue ? 'habilitado' : 'deshabilitado'}`);
        } catch (error) {
    ...
    const handleToggleOnlineVisibility = async () => {
        try {
            ...
            toast.success(`Visibilidad en línea ${newValue ? 'habilitada' : 'deshabilitada'}`);
        } catch (error) {
    ...
    const handleToggleAutoplayVideos = async () => {
        try {
            ...
            window.dispatchEvent(new CustomEvent('autoplaySettingChanged', { 
                detail: { enabled: newValue } 
            }));
            
            toast.success(`Reproducción automática de videos ${newValue ? 'habilitada' : 'deshabilitada'}`);
        } catch (error) {
            ...
            <div className="containerSettingItems">
                ...
                <div className="itemSetting">
                    ...
                    <div className="privacy-settings mt-2">
                        <p className="mb-3">Controla quién puede ver tu perfil y tu información personal.</p>
                        <div className="flex flex-col space-y-2">
                            <button
                                onClick={() => handlePrivacyChange('publico')}
                                className={`flex items-center gap-2 p-2 rounded-md ${
                                    privacy === 'publico' ? 'bg-purple-100 text-purple-600' : 'hover:bg-gray-100'
                                }`}
                                disabled={loading}
                            >
                                <FiGlobe size={18} />
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 p-2 rounded-md ${
                                    privacy === 'amigos' ? 'bg-purple-100 text-purple-600' : 'hover:bg-gray-100'
                                }`}
                                disabled={loading}
                            >
                                <FiUsers size={18} />
                                <div className="flex flex-col items-start">
                                    ...
                                    <span className="text-xs text-gray-500">Solo tus amigos pueden ver tu perfil</span>
                                </div>
                            </button>
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 p-2 rounded-md ${
                                    privacy === 'privado' ? 'bg-purple-100 text-purple-600' : 'hover:bg-gray-100'
                                }`}
                                disabled={loading}
                            >
                                <FiLock size={18} />
                                <div className="flex flex-col items-start">
                                    ...
                                    <span className="text-xs text-gray-500">Solo tú puedes ver tu perfil</span>
                                </div>
                            </button>
                        </div>
                    </div>
                ...
                <div className="itemSetting">
                    ...
                    <div className="itemSettingContent">
                        <p>Permitir que la aplicación acceda a tus archivos para compartir.</p>
                        <label className="switch">
                            <input 
                                type="checkbox" 
                ...
                <div className="itemSetting">
                    ...
                    <div className="itemSettingContent">
                        <p>Reproducir videos automáticamente cuando se envían desde otros usuarios.</p>
                        <label className="switch">
                            <input
                                type="checkbox"
