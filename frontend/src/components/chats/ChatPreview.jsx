...
const ChatPreview = ({ friend, onSelect, isSelected }) => {
  ...
  const friendId =
    friend.id_usuario1 === parseInt(currentUserId)
      ? friend.id_usuario2
      : friend.id_usuario1;
  ...
  useEffect(() => {
    ...
    if (socketRef.current) {
      ...
      socketRef.current.on("receive_private_message", (message) => {
        if (
          (message.senderId === parseInt(friendId) &&
            message.receiverId === parseInt(currentUserId)) ||
          (message.senderId === parseInt(currentUserId) &&
            message.receiverId === parseInt(friendId))
        ) {
          // Update last message
    ...
    const fetchAvatar = async () => {
      try {
        const response = await fetch(
          `http://localhost:3009/users/usuario/${friendId}`
        );
        ...
        if (userData.estado) {
          setConnectionStatus(
            userData.estado === "conectado" ? "conectado" : "desconectado"
          );
        }
      } catch (error) {
  ...
  const markMessagesAsRead = async () => {
    try {
      await fetch(
        `http://localhost:3001/chats/markAsRead/${currentUserId}/${friendId}`,
        {
          method: "PUT",
        }
      );
  ...
  const fetchLastMessage = async () => {
    try {
      const response = await fetch(
        `http://localhost:3001/chats/messages/${currentUserId}/${friendId}`
      );
  ...
  const fetchUnreadCount = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/chats/unread/${currentUserId}/${friendId}`
      );
      ...
      <div className="flex items-center space-x-5">
        <div className="imgstado">
          <img
            src={avatarUrl}
            alt={
              friend.nombre_usuario2 || friend.nombre_usuario1 || "User avatar"
            }
            className="chat-avatar"
            onError={() => setAvatarUrl(avatarDefault)}
          />
          <span
            className={`status-indicator-chat ${
        ...
        <div className="flex-1 PreviewChat">
          ...
          <p className="chat-message truncate">{lastMessage}</p>
        </div>
      </div>
    </div>
  );
