...
const SidebarComments = ({
    ...
    const fetchComments = async () => {
      ...
      try {
        ...
        if (fetchedComments && fetchedComments.length > 0) {
          const uniqueUserIds = [
            ...new Set(fetchedComments.map((comment) => comment.id_usuario)),
          ];
    ...
    const fetchRepliesForComment = async (commentId) => {
      try {
        ...
        if (replies && replies.length > 0) {
          ...
          const uniqueUserIds = [
            ...new Set(replies.map((reply) => reply.id_usuario)),
          ];
          ...
          for (const replyUserId of uniqueUserIds) {
            try {
                ...
                replies = replies.map((reply) => {
                  if (reply.id_usuario === replyUserId) {
                    return {
                      ...reply,
                      nombre_usuario:
                        userProfile.nombre || userProfile.username,
              ...
              console.error(
                `Error fetching user profile for reply user ID ${replyUserId}:`,
                error
              );
            }
          }
      ...
      if (createdComment) {
        try {
          ...
          const formattedComment = {
            ...
            nombre_usuario:
              userProfile.nombre || userProfile.username || "Usuario",
            avatar_usuario: userProfile.avatar,
            contenido: newComment.trim(),
          ...
          console.error(
            "Error fetching user profile for new comment:",
            profileError
          );
  ...
  return (
    <div
      className={`sidebar-comments-wrapper ${
        isSidebarVisible ? "open" : "closed"
      }`}
    >
      <button
        className="button-close-comments"
        onClick={() => {
                        ...
                        <div className="comment-content">
                          <div className="comment-header">
                            <div className="comment-author">
                              {comment.nombre_usuario || "Usuario"}
                            </div>
